...
from alpha_util.const import FREQ_NUM
from alpha_util.util import timethis
...
def cut_group(df, factor_name, num_group, ascending):
    ...
    """
    # N = len(df)
    # bins = np.linspace(0, N, num_group + 1)
    # idx=None
    # try:
    #     idx = df.loc[:, factor_name].rank(method='first', na_option='drop',
    #                                   ascending=ascending) - 1
    # except:
    #     print ('no')
    # digits_group = np.digitize(idx, bins)
...
def phil_return_analysis(factor_name, grouped, freq, benchmark_term_return,
    ...
    try:
        ret_mean.loc[:, 'Q_LS'] = ret_mean[str('Q1')] - ret_mean[str(g_sell)]
    ...
    # 夏普比率(无风险利率采用3%）
    df_stats['sharp_ratio'] = (df_stats['r_cum_y'] - 0.03) / df_stats['sigma_y']
    ...
    df_stats['alpha'] = [
        sm.OLS(ret_mean.T[col].dropna().values, x.loc[ret_mean.T[col].dropna().index, :].values).fit().params[0]
        if not ret_mean.T[col].isnull().all() else np.nan for col in ret_mean.T.columns]
    df_stats['beta'] = [
        sm.OLS(ret_mean.T[col].dropna().values, x.loc[ret_mean.T[col].dropna().index, :].values).fit().params[1]
        if not ret_mean.T[col].isnull().all() else np.nan for col in ret_mean.T.columns]
...
def __calc_percent(dic):
    ...
    ret = {k: v / total for k, v in dic.items()}
    return ret
