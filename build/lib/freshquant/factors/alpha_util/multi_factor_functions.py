...
def __maxIR_weighted_scoring(factor_names, standardized_factor, single_factor_analysis_results, score_window,
                             fac_ret_gp="Q_LS",biggest_best=None):
    if score_window is None:
        score_window = 12
    ...
    ic=pd.DataFrame(ic)
    cov = np.cov(ic)
    ic = np.array(ic.mean(axis=1))
    n = len(ic)  # n是因子个数
    W = np.ones([n])/n
    target = lambda W: 1 / ((sum(W * ic)) / math.sqrt(np.dot(np.dot(W, cov), W)))
    b = [(0., 1.) for i in range(n)]
    c = ({'type': 'eq', 'fun': lambda W: sum(W) - 1.})   # Constraint type: ‘eq’ for equality, ‘ineq’ for inequality
    optimized = scipy.optimize.minimize(target, W, method='SLSQP', constraints=c, bounds=b)
    weights = optimized.x
...
def factor_scoring(factor_names, standardized_factor, single_factor_analysis_results, score_method='eqwt',
    ...
    valid_methods = {'eqwt': __equal_weighted_scoring,
                     ...
                     'maxIRwt':__maxIR_weighted_scoring}
