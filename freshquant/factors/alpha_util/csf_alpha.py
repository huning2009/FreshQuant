...
from alpha_util.get_data import (get_benchmark_return, get_raw_factor,
                                       get_term_return,get_cap_data)
from alpha_util.neutral import neutral
"""
多因子选股数据接口
"""
import pickle
import alpha_util.util as ut
...
class CSFAlpha(object):
    ...
    # TODO dynamic modelling

    def __init__(self,codes,factor_codes,start_date,end_date,
                 bench_code,fac_ret_cap=None,neutral=True,freq='m',isIndex=True,turnover_method='count',
        ...
        self.dt_index = form_dt_index(start_date, end_date, freq)  # 调仓日期
        # 每个调仓期的股票代码，字典格式
        ...
        self.fac_ret_cap=fac_ret_cap
        # 存放所有因子测试的详细结果
        self.single_factor_analysis_results = dict(
            list(zip(self.factor_codes, [FactorData(None,
                                               ReturnAnalysis(),
                                               ICAnalysis(),
                                               TurnOverAnalysis(),
                                               CodeAnalysis())] * len(
    ...
    def _get_all_stock(self):
        ...
        """
        # codes = []
        # for k, v in self.term_stock_codes.iteritems():
        #     codes.extend(v)
        # ret = list(set(codes))
        # return ret
        #
    ...
    def _get_idx_his_stock(self):
        ...
        # return ret
        rets = []
        if self.codes not in ['HOT']:
            for dt in self.dt_index[:-1]:
                ret = ut.get_index_components(self.codes, dt)
                ret = list(zip([dt] * len(ret), ret))
                rets.extend(ret)
        else:
            client, db, tb = csf_utils.get_mongo_connection('122.144.134.4', 'ada', 'dict_index')
            filters = {'serie':'HOT'}
    ...
    def _get_raw_factor(self):
        ...
        factor_codes_dict = dict(list(zip(list(self.codes_factor_dict.values()), list(self.codes_factor_dict.keys()))))
    ...
    def combine_fac_ret_cap(self):
        ...
        if self.neutral is True:
            combined=neutral(combined)
    ...
    def get_all_data(self):
        ...
        综合数据: df_com_facs
        """
        if self.fac_ret_cap is not None:
            self._get_benchmark_return()
            fac_ret_cap=self.fac_ret_cap.loc[self.term_stock_codes]
            field=[col for col in fac_ret_cap.columns if col in self.factor_codes or col in ['ret','cap']]
            self.all_data['fac_ret_cap']=fac_ret_cap[field]
        else:
    ...
    def _regress_analysis(self, df_com_fac):
        ...
        """
        ret = OrderedDict()
        for dt, df_v in df_com_fac.items():
    ...
    def factor_scoring(self, fac_names, score_method='eqwt', score_window=12,
        ...
        key0 = list(self.single_factor_analysis_results.keys())[0]
        ...
        results={k: v for k, v in results.items() if k in fac_names}
    ...
    def parallel_run_single_factor_analysis(self, n_jobs=1):
        ...
        ret = dict(list(zip(self.factor_codes, ret)))
    ...
    def return_analysis(self, n_jobs=1):
        ret = Parallel(n_jobs=n_jobs)(
            delayed(return_analysis)(
                ...
                return_mean_method=self.return_mean_method)
            for factor_name in self.factor_codes)
        return_analysis_dic = dict(list(zip(self.factor_codes, ret)))
        for factor_name in self.factor_codes:
    ...
    def IC_analysis(self, n_jobs=1):
        ret = Parallel(n_jobs=n_jobs)(
            delayed(ic_analysis)(factor_name,
                                 ...
                                 ic_method=self.ic_method)
            for factor_name in self.factor_codes)
        ic_analysis_dic = dict(list(zip(self.factor_codes, ret)))
        for factor_name in self.factor_codes:
            self.single_factor_analysis_results[
    ...
    def turnover_analysis(self, n_jobs=1):
        ret = Parallel(n_jobs=n_jobs)(
            delayed(turnover_analysis)(
                ...
                turnover_method=self.turnover_method)
            for factor_name in self.factor_codes)
        turnover_analysis_dic = dict(list(zip(self.factor_codes, ret)))
        for factor_name in self.factor_codes:
            self.single_factor_analysis_results[
    ...
    def code_analysis(self, n_jobs=1):
        ...
        code_analysis_dic = dict(list(zip(self.factor_codes, ret)))
        for factor_name in self.factor_codes:
            self.single_factor_analysis_results[
...
if __name__ == '__main__':
    factor_codes = ['M004023']
    start_date = '2009-04-30'  # 开始日期pd.concat([ret,ret2],axis=1)
    end_date = '2011-03-30'  # 结束日期
    bench_code = '000905'  # benchmark代码
    ins = CSFAlpha(bench_code, factor_codes, start_date, end_date,
                   bench_code=bench_code,neutral=False,freq='m', isIndex=True, num_group=5,
                   g_sell='Q5')
    ins.get_all_data()
    all_data=ins.all_data
    # results=ins.parallel_run_single_factor_analysis(n_jobs=1)
    # ins.all_data['fac_ret_cap'].xs('2008-05-30').to_csv('C:\Users\jessica.sun\Desktop\Test\Fac_ret_cap.csv')
    # ins.all_data['df_raw_factor'].xs('2008-05-30').to_csv('C:\Users\jessica.sun\Desktop\Test\Raw.csv')
    ins.multi_factor_analysis(fac_names=factor_codes,num_group=5,comb_name='comb',score_method='eqwt',score_window=None,biggest_best=None)
    ret=ins.multi_factor_analysis_results
    stocks=ret['comb'].code_analysis.stock_list
    stocks_top={}
    for (k1,k2),v in stocks.items():
        if k2=='Q1':
            stocks_top[k1]=v
    print('Done!')
    ...
    # end_date = '2015-07-31'  # 结束日期
    # bench_code = 'HOT'  # benchmark代码
    # ins = CSFAlpha(bench_code, factor_codes, start_date, end_date,
    #                bench_code=bench_code,neutral=False,freq='m', isIndex=True, num_group=10,
    #                g_sell='Q10')
    # ins.get_all_data()
    # # ins.all_data['fac_ret_cap'].xs('2008-05-30').to_csv('C:\Users\jessica.sun\Desktop\Test\Fac_ret_cap.csv')
    # # ins.all_data['df_raw_factor'].xs('2008-05-30').to_csv('C:\Users\jessica.sun\Desktop\Test\Raw.csv')
    # ins.multi_factor_analysis(fac_names=factor_codes,num_group=10,comb_name='comb',score_method='eqwt',score_window=None,biggest_best=None)
    # ret=ins.multi_factor_analysis_results
    # print 'Done!'
